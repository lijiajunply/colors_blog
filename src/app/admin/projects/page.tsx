...
async function getProjects(search?: string, page: number = 1, pageSize: number = 10) {
...
export default async function ProjectsPage({ searchParams }: { searchParams?: { search?: string; page?: string } }) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">管理你的项目</p>
        </div>
        <Link
            ...
            {projects.map((project: Project) => (
              <tr key={project.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${project.isActive ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <form action={() => deleteProject(project.id)} method="post" className="inline">
                    <button
                      type="submit"
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          <Link
            href={{ pathname: '/admin/projects', query: { ...searchParams, page: Math.max(1, page - 1) } }}
          ...
          <Link
            href={{ pathname: '/admin/projects', query: { ...searchParams, page: Math.min(totalPages, page + 1) } }}
            className="px-3 py-1 border border-gray-300 dark:border-gray-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700"
          >
            下一页
          </Link>
